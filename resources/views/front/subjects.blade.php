...
@section('content')
    @php
        ...
        $getFilter = function ($subject) {
            ...
            if (str_contains($stageName, 'بكالوريا')) {
                return 'بكالوريا';
            }
            if (str_contains($stageName, 'ثانوي')) {
                return 'ثانوي';
            }
            if (str_contains($stageName, 'اعداد')) {
                return 'اعدادي';
            }
            return '';
        };
        ...
        <div class="subj-hero-inner">
            <div>
                ...
                <div class="hero-pills">
                    <div class="hero-pill {{ ($activeStage ?? 'الكل') === 'الكل' ? 'active' : '' }}" data-filter="الكل">الكل
                    </div>
                    <div class="hero-pill {{ ($activeStage ?? 'الكل') === 'اعدادي' ? 'active' : '' }}" data-filter="اعدادي">
                        اعدادي</div>
                    <div class="hero-pill {{ ($activeStage ?? 'الكل') === 'ثانوي' ? 'active' : '' }}" data-filter="ثانوي">
                        ثانوي</div>
                    <div class="hero-pill {{ ($activeStage ?? 'الكل') === 'بكالوريا' ? 'active' : '' }}"
                        data-filter="بكالوريا">بكالوريا</div>
                </div>
            </div>
        ...
        <div class="subj-grid" id="subj-grid">
            @forelse ($subjects ?? [] as $index => $subject)
                ...
                <a href="{{ route('front.courses.subject', $subject) }}"
                    class="subj-card {{ $theme }} subj-card-item" data-filter="{{ $filterVal }}"
                    data-name="{{ $subject->name }}">
                    <div class="subj-card-fav" onclick="event.preventDefault(); event.stopPropagation();">
                        @include('front.components.favorite-btn', [
                            'subject' => $subject,
                            'isFavorite' => in_array($subject->id, $favoriteSubjectIds ?? []),
                        ])
                    </div>
                    <div class="subj-thumb">
                        @if ($subject->image)
                            <img src="{{ display_file($subject->image) }}" alt="{{ $subject->name }}"
                                class="subj-thumb-img">
                        @endif
                        @if (!$subject->image)
                    ...
                    <div class="subj-body">
                        ...
                        <div class="subj-desc">كورس كامل منهج المادة مع حصص فيديو وتقييمات.</div>
                        ...
                        <div class="subj-stats">
                            <span class="subj-stat">
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="none"
                                    ...
                                    <path
                                        d="M15 10l4.553-2.069A1 1 0 0 1 21 8.845v6.31a1 1 0 0 1-1.447.894L15 14M3 8a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" />
                                </svg>
                                24 فيديو
                            </span>
                            <span class="subj-stat">
            ...
            (function() {
                ...
                function applyFilter() {
                    ...
                    cards.forEach(function(card) {
                        ...
                        var showBySearch = !q || (card.getAttribute('data-name') || '').toLowerCase().indexOf(q) !==
                            -1;
