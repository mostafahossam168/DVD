...
<div class="dash-page">
    <div class="page-breadcrumb fade-up-ds">
        ...
        <span class="sep">/</span>
        <span class="current">المعلمين</span>
    </div>
    ...
    <div class="page-header-ds fade-up-ds">
        ...
        <div class="page-header-actions">
            @can('create_teachers')
                <a href="{{ route('dashboard.teachers.create') }}" class="btn-add-ds">
    ...
    <div class="filters-bar-ds fade-up-ds delay-1-ds">
        <div class="filters-right-ds">
            <a href="{{ route('dashboard.teachers.index') }}" class="filter-badge-ds {{ !request('status') ? 'active-ds' : '' }}">
                الكل <span class="badge-count-ds">{{ $count_all }}</span>
            </a>
            <a href="{{ route('dashboard.teachers.index', ['status' => 'yes']) }}" class="filter-badge-ds enabled-ds {{ request('status') === 'yes' ? 'active-ds' : '' }}">
                مفعلين <span class="badge-count-ds">{{ $count_active }}</span>
            </a>
            <a href="{{ route('dashboard.teachers.index', ['status' => 'no']) }}" class="filter-badge-ds disabled-ds {{ request('status') === 'no' ? 'active-ds' : '' }}">
                غير مفعلين <span class="badge-count-ds">{{ $count_inactive }}</span>
            </a>
        </div>
        <div class="filters-left-ds">
            <form action="{{ route('dashboard.teachers.index') }}" method="get" class="search-wrap-ds">
            ...
            <tbody>
                @forelse($items as $item)
                    <tr>
                        ...
                        <td><span style="font-weight:800">{{ $item->full_name ?? $item->fullname ?? '—' }}</span></td>
                        ...
                        <td><span class="status-badge-ds" style="background:#f1f5f9;color:#64748b">{{ $item->roles->first()?->name ?? '—' }}</span></td>
                        <td>
                            <div class="actions-cell-ds">
                                @can('read_teachers')
                                    <a href="{{ route('dashboard.teachers.show', $item->id) }}" class="action-btn-ds edit-ds" title="عرض">
                                ...
                                @endcan
                                @can('update_teachers')
                                    <a href="{{ route('dashboard.teachers.edit', $item->id) }}" class="action-btn-ds edit-ds" title="تعديل">
                ...
                @empty
                    <tr>
                        <td colspan="8" style="text-align:center;padding:3rem;color:var(--muted);font-weight:600">لا يوجد معلمون</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
