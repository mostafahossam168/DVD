...
<div class="dash-page">
    <div class="page-breadcrumb fade-up-ds">
        ...
        <span class="sep">/</span>
        <span class="current">الصلاحيات</span>
    </div>
    ...
    <div class="page-header-ds fade-up-ds">
        ...
        <div class="page-header-actions">
            @can('create_roles')
                <a href="{{ route('dashboard.roles.create') }}" class="btn-add-ds">
    ...
    <div class="filters-bar-ds fade-up-ds delay-1-ds">
        <div class="filters-right-ds">
            <a href="{{ route('dashboard.roles.index') }}" class="filter-badge-ds active-ds">
                الكل <span class="badge-count-ds">{{ \Spatie\Permission\Models\Role::count() }}</span>
            </a>
        </div>
        <div class="filters-left-ds">
            <form action="{{ route('dashboard.roles.index') }}" method="get" class="search-wrap-ds">
    ...
    <div class="table-wrap-ds fade-up-ds delay-2-ds">
        <table>
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="actions-cell-ds">
                                @can('read_roles')
                                    <a href="{{ route('dashboard.roles.show', $item->id) }}" class="action-btn-ds edit-ds" title="عرض">
                                ...
                                @endcan
                                @can('update_roles')
                                    <a href="{{ route('dashboard.roles.edit', $item->id) }}" class="action-btn-ds edit-ds" title="تعديل">
                                ...
                                @endcan
                                @if(\Spatie\Permission\Models\Role::first() && \Spatie\Permission\Models\Role::first()->id != $item->id)
                                    @can('delete_roles')
                ...
                @empty
                    <tr>
                        <td colspan="3" style="text-align:center;padding:3rem;color:var(--muted);font-weight:600">لا توجد صلاحيات</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
