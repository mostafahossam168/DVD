@extends('dashboard.layouts.backend', ['title' => 'أسئلة التقييمات'])

@section('contant')
<div class="dash-page">
    <div class="page-breadcrumb fade-up-ds">
        <a href="{{ route('dashboard.home') }}">الرئيسية</a>
        <span class="sep">/</span>
        <span class="current">أسئلة التقييمات</span>
    </div>

    <div class="page-header-ds fade-up-ds">
        <h1>أسئلة التقييمات</h1>
        <div class="page-header-actions">
            @can('create_question_bank')
                <a href="{{ route('dashboard.question-bank.create') }}" class="btn-add-ds">إضافة سؤال</a>
            @endcan
        </div>
    </div>

    <x-alert-component></x-alert-component>

    <div class="filters-bar-ds fade-up-ds delay-1-ds">
        <div class="filters-right-ds">
            <a href="{{ route('dashboard.question-bank.index') }}" class="filter-badge-ds {{ !request('status') ? 'active-ds' : '' }}">
                الكل <span class="badge-count-ds">{{ $count_all }}</span>
            </a>
            <a href="{{ route('dashboard.question-bank.index', array_filter(['status' => 'yes', 'type' => request('type'), 'difficulty' => request('difficulty')])) }}" class="filter-badge-ds enabled-ds {{ request('status') === 'yes' ? 'active-ds' : '' }}">
                مفعل <span class="badge-count-ds">{{ $count_active }}</span>
            </a>
            <a href="{{ route('dashboard.question-bank.index', array_filter(['status' => 'no', 'type' => request('type'), 'difficulty' => request('difficulty')])) }}" class="filter-badge-ds disabled-ds {{ request('status') === 'no' ? 'active-ds' : '' }}">
                غير مفعل <span class="badge-count-ds">{{ $count_inactive }}</span>
            </a>
            <form action="{{ route('dashboard.question-bank.index') }}" method="GET" style="display:inline-flex;align-items:center;gap:8px;margin-inline-start:10px;flex-wrap:wrap">
                @if(request('status'))<input type="hidden" name="status" value="{{ request('status') }}">@endif
                <select name="type" class="form-control form-control-sm filter-select-ds" style="width:auto;min-width:140px" onchange="this.form.submit()">
                    <option value="">النوع</option>
                    <option value="mcq" @selected(request('type') == 'mcq')>اختياري</option>
                    <option value="true_false" @selected(request('type') == 'true_false')>صح/خطأ</option>
                    <option value="text" @selected(request('type') == 'text')>مقالي</option>
                </select>
                <select name="difficulty" class="form-control form-control-sm filter-select-ds" style="width:auto;min-width:140px" onchange="this.form.submit()">
                    <option value="">الصعوبة</option>
                    <option value="easy" @selected(request('difficulty') == 'easy')>سهل</option>
                    <option value="medium" @selected(request('difficulty') == 'medium')>متوسط</option>
                    <option value="hard" @selected(request('difficulty') == 'hard')>صعب</option>
                </select>
            </form>
        </div>
        <div class="filters-left-ds">
            <form action="{{ route('dashboard.question-bank.index') }}" method="GET" class="search-wrap-ds">
                @if(request('status'))<input type="hidden" name="status" value="{{ request('status') }}">@endif
                @if(request('type'))<input type="hidden" name="type" value="{{ request('type') }}">@endif
                @if(request('difficulty'))<input type="hidden" name="difficulty" value="{{ request('difficulty') }}">@endif
                <input type="search" name="search" value="{{ request('search') }}" placeholder="بحث في الأسئلة...">
            </form>
        </div>
    </div>

    <div class="table-wrap-ds fade-up-ds delay-2-ds">
        <table>
            <thead>
                <tr>
                    <th>#</th>
                    <th>السؤال</th>
                    <th>النوع</th>
                    <th>المرحلة</th>
                    <th>الصف</th>
                    <th>المادة</th>
                    <th>الصعوبة</th>
                    <th>الدرجة الافتراضية</th>
                    <th>التصنيفات</th>
                    <th>الحالة</th>
                    <th>العمليات</th>
                </tr>
            </thead>
            <tbody>
                @forelse($items as $item)
                    <tr>
                        <td class="td-num-ds">{{ $loop->iteration }}</td>
                        <td style="max-width:420px">{{ \Illuminate\Support\Str::limit($item->question_text, 100) }}</td>
                        <td>{{ $item->type }}</td>
                        <td>{{ $item->stage?->name ?? '—' }}</td>
                        <td>{{ $item->grade?->name ?? '—' }}</td>
                        <td>{{ $item->subject?->name ?? '—' }}</td>
                        <td>{{ $item->difficulty }}</td>
                        <td>{{ $item->default_mark ?? '—' }}</td>
                        <td>
                            @forelse($item->categories as $cat)
                                <span class="status-badge-ds" style="margin:2px">{{ $cat->name }}</span>
                            @empty
                                —
                            @endforelse
                        </td>
                        <td>
                            @if($item->status)
                                <span class="status-badge-ds enabled-ds">مفعل</span>
                            @else
                                <span class="status-badge-ds disabled-ds">غير مفعل</span>
                            @endif
                        </td>
                        <td>
                            <div class="actions-cell-ds">
                                <a href="{{ route('dashboard.question-bank.show', $item->id) }}" class="action-btn-ds" title="عرض">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7S1 12 1 12z"/><circle cx="12" cy="12" r="3"/></svg>
                                </a>
                                @can('update_question_bank')
                                <a href="{{ route('dashboard.question-bank.edit', $item->id) }}" class="action-btn-ds edit-ds" title="تعديل">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                </a>
                                @endcan
                                @can('delete_question_bank')
                                <button type="button" class="action-btn-ds delete-ds" data-bs-toggle="modal" data-bs-target="#delete{{ $item->id }}" title="حذف">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>
                                </button>
                                @endcan
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="11" style="text-align:center;padding:2rem">لا توجد أسئلة في البنك</td></tr>
                @endforelse
            </tbody>
        </table>
        @if($items->hasPages())
            <div style="padding:1rem 1.5rem;border-top:1px solid var(--border)">{{ $items->withQueryString()->links() }}</div>
        @endif
    </div>
</div>
@endsection

@push('modals')
    @foreach($items as $item)
        @can('delete_question_bank')
            @include('dashboard.question-bank.delete-model', ['item' => $item])
        @endcan
    @endforeach
@endpush
