...
@section('contant')
    <div class="main-side">
        ...
        <form action="{{ route('dashboard.lectuers.update', $item->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row g-4">
                ...
                <div class="col-12 col-md-4 col-lg-3">
                    ...
                    <select name="stage_id" id="tax" class="form-select select-setting">
                        ...
                        @foreach ($stages as $stage)
                            <option value="{{ $stage->id }}" @selected($item->subject->grade->stage->id == $stage->id)>{{ $stage->name }}</option>
                        @endforeach
                    </select>
                    @error('stage_id')
                        <span class="text-danger">{{ $message }}</span>
                ...
                <div class="col-12 col-md-4 col-lg-3">
                    ...
                    <select name="grade_id" id="tax" class="form-select select-setting">
                        <option value="{{ $item->subject->grade->id }}">{{ $item->subject->grade->name }}
                        </option>
                    </select>
                    @error('grade_id')
                        <span class="text-danger">{{ $message }}</span>
                ...
                <div class="col-12 col-md-4 col-lg-3">
                    ...
                    <select name="subject_id" id="tax" class="form-select select-setting">
                        <option value="{{ $item->subject->id }}">{{ $item->subject->name }}
                        </option>
                    </select>
                    @error('subject_id')
                        <span class="text-danger">{{ $message }}</span>
                ...
                <div class="col-12 col-md-4 col-lg-3">
                    <label class="special-input">
                        <span>اللينك</span>
                        <div class="box-input">
                            <input type="url" name="link" value="{{ $item->link }}" id="">
                        </div>
                        @error('link')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                ...
                <div class="col-12 ">
                    ...
                    <textarea name="description" id="" class="form-control" cols="150" rows="10">{{ $item->description }}</textarea>
                    @error('description')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
