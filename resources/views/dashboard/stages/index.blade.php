@extends('dashboard.layouts.backend', ['title' => 'المراحل الدراسية'])
...
<div class="dash-page">
    <div class="page-breadcrumb fade-up-ds">
        ...
        <span class="sep">/</span>
        <span class="current">المراحل الدراسية</span>
    </div>
    ...
    <div class="filters-bar-ds fade-up-ds delay-1-ds">
        <div class="filters-right-ds">
            <a href="{{ route('dashboard.stages.index') }}" class="filter-badge-ds {{ !request('status') ? 'active-ds' : '' }}">
                الكل <span class="badge-count-ds">{{ $count_all }}</span>
            </a>
            <a href="{{ route('dashboard.stages.index', ['status' => 'yes']) }}" class="filter-badge-ds enabled-ds {{ request('status') === 'yes' ? 'active-ds' : '' }}">
                مفعل <span class="badge-count-ds">{{ $count_active }}</span>
            </a>
            <a href="{{ route('dashboard.stages.index', ['status' => 'no']) }}" class="filter-badge-ds disabled-ds {{ request('status') === 'no' ? 'active-ds' : '' }}">
                غير مفعل <span class="badge-count-ds">{{ $count_inactive }}</span>
            </a>
        </div>
        <div class="filters-left-ds">
            <form action="{{ route('dashboard.stages.index') }}" method="get" class="search-wrap-ds">
            ...
            <tbody>
                @forelse($items as $item)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('dashboard.grades.index', ['stage_id' => $item->id]) }}" class="count-badge-ds {{ $item->grades->count() > 0 ? 'high-ds' : 'zero-ds' }}">{{ $item->grades->count() }}</a>
                        </td>
                        <td>
                ...
                @empty
                    <tr>
                        <td colspan="5" style="text-align:center;padding:3rem;color:var(--muted);font-weight:600">لا توجد مراحل</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
@push('modals')
    ...
    @foreach($items as $item)
        @can('delete_stages')@include('dashboard.stages.delete-modal', ['item' => $item])@endcan
        @can('update_stages')@include('dashboard.stages.edit-modal', ['item' => $item])@endcan
    @endforeach
@endpush
