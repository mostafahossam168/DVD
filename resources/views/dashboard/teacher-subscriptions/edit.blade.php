...
@section('contant')
    <div class="main-side">
        ...
        <form action="{{ route('dashboard.teacher-subscriptions.update', $item->id) }}" method="post">
            @csrf
            @method('PUT')
            <div class="row g-4">
                <div class="col-12 col-md-6">
                    <label class="special-label" for="user_id">
                        المدرس</label>
                    <select name="user_id" id="user_id" class="form-select select-setting" required @disabled(auth()->user()->type === 'teacher' && !auth()->user()->hasRole('admin'))>
                        <option value="">-- اختر المدرس --</option>
                        @foreach($teachers as $teacher)
                            <option value="{{ $teacher->id }}" @selected($item->user_id == $teacher->id || old('user_id') == $teacher->id)>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="plan_id" id="plan_id" class="form-select select-setting" required>
                        ...
                        @foreach($plans as $plan)
                            <option value="{{ $plan->id }}" @selected($item->plan_id == $plan->id || old('plan_id') == $plan->id)>
