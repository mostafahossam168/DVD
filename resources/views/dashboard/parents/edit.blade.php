@extends('dashboard.layouts.backend', ['title' => 'تعديل ولي أمر'])
...
<div class="dash-page">
    <div class="page-breadcrumb fade-up-ds">
        ...
        <span class="sep">/</span>
        <a href="{{ route('dashboard.parents.index') }}">أولياء الأمور</a>
        <span class="sep">/</span>
        <span class="current">تعديل ولي أمر</span>
    </div>
    ...
    <a href="{{ route('dashboard.parents.index') }}" class="btn-back-ds fade-up-ds">
    ...
    <form action="{{ route('dashboard.parents.update', $item->id) }}" method="post" enctype="multipart/form-data" class="fade-up-ds delay-1-ds">
        @csrf
        @method('PUT')
        <div class="form-card-ds">
            <div class="form-card-header-ds">
                ...
                <div>
                    ...
                    <p>تعديل معلومات {{ $item->full_name ?? $item->email }}</p>
                </div>
            </div>
            <div class="form-card-body-ds">
                ...
                <div class="form-divider-ds">الأبناء</div>
                <div class="form-grid-ds">
                    <div class="form-group-ds" style="grid-column:1/-1">
                        <label class="form-label-ds">اختر الأبناء (طلاب)</label>
                        <select name="children[]" class="form-control-ds" multiple size="6">
                            @foreach($students as $student)
                                <option value="{{ $student->id }}" @selected(collect(old('children', $selectedChildren))->contains($student->id))>
                                    {{ $student->full_name }} ({{ $student->email }})
                                </option>
                            @endforeach
                        </select>
                        @error('children')<span class="form-error-ds" style="font-size:.76rem;color:#dc2626">{{ $message }}</span>@enderror
                    </div>
                </div>
            ...
            <div class="form-card-footer-ds">
                ...
                <a href="{{ route('dashboard.parents.index') }}" class="btn-ds btn-secondary-ds">إلغاء</a>
            </div>
        </div>
    </form>
</div>
