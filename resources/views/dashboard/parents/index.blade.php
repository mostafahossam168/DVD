@extends('dashboard.layouts.backend', ['title' => 'أولياء الأمور'])
...
<div class="dash-page">
    <div class="page-breadcrumb fade-up-ds">
        ...
        <span class="sep">/</span>
        <span class="current">أولياء الأمور</span>
    </div>
    ...
    <div class="page-header-ds fade-up-ds">
        ...
        <div class="page-header-actions">
            @can('create_parents')
                <a href="{{ route('dashboard.parents.create') }}" class="btn-add-ds">
    ...
    <div class="filters-bar-ds fade-up-ds delay-1-ds">
        <div class="filters-right-ds">
            <a href="{{ route('dashboard.parents.index') }}" class="filter-badge-ds {{ !request('status') ? 'active-ds' : '' }}">
                الكل <span class="badge-count-ds">{{ $count_all }}</span>
            </a>
            <a href="{{ route('dashboard.parents.index', ['status' => 'yes']) }}" class="filter-badge-ds enabled-ds {{ request('status') === 'yes' ? 'active-ds' : '' }}">
                مفعلين <span class="badge-count-ds">{{ $count_active }}</span>
            </a>
            <a href="{{ route('dashboard.parents.index', ['status' => 'no']) }}" class="filter-badge-ds disabled-ds {{ request('status') === 'no' ? 'active-ds' : '' }}">
                غير مفعلين <span class="badge-count-ds">{{ $count_inactive }}</span>
            </a>
        </div>
        <div class="filters-left-ds">
            <form action="{{ route('dashboard.parents.index') }}" method="get" class="search-wrap-ds">
    ...
    <div class="table-wrap-ds fade-up-ds delay-2-ds">
        <table>
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="actions-cell-ds">
                                @can('update_parents')
                                    <a href="{{ route('dashboard.parents.edit', $item->id) }}" class="action-btn-ds edit-ds" title="تعديل">
                ...
                @empty
                    <tr>
                        <td colspan="8" style="text-align:center;padding:3rem;color:var(--muted);font-weight:600">لا يوجد أولياء أمور</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
@push('modals')
    @foreach($items as $item)
        @can('delete_parents')
            @include('dashboard.parents.delete-model', ['item' => $item])
        @endcan
    @endforeach
@endpush
