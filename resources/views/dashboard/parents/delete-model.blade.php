<div class="modal fade" id="delete{{ $item->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('dashboard.parents.destroy', $item->id) }}" method="POST">
                @csrf
                @method('DELETE')
                <div class="confirm-body-ds">
                    <div class="confirm-icon-ds danger-ds">⚠️</div>
                    <h3>هل أنت متأكد من الحذف؟</h3>
                    <p>سيتم حذف ولي الأمر «{{ $item->full_name ?? $item->email }}» بشكل نهائي ولا يمكن التراجع عن هذا الإجراء.</p>
                </div>
